      ...
      <div class="form-table-box">
        <el-form ref="infoForm" :model="infoForm" :rules="infoRules" label-width="120px">
          <el-form-item label="打印后自动发货">
            <el-switch v-model="infoForm.autoDelivery" @change="changeStatus(infoForm.autoDelivery)"></el-switch>
          </el-form-item>
        ...
        if (valid) {
          this.axios.post('admin/storeShipperSettings', this.infoForm).then(response => {
            if (response.data.errno === 0) {
              ...
              this.infoForm.autoDelivery == 1
                ? (this.infoForm.autoDelivery = true)
                : (this.infoForm.autoDelivery = false)
            } else {
              this.$message({
                type: 'error',
                message: '保存失败'
